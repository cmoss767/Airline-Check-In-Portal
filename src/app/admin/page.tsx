...
export default function AdminPage() {
  ...
  if (error) return <p className="p-8 text-center text-brand-red">Error: {error.message}</p>;
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full bg-white rounded-lg shadow">
          <thead className="bg-gray-100">
            <tr>
              <th className="px-6 py-3 text-left text-xs font-medium text-brand-text uppercase tracking-wider">Passenger</th>
              <th className="px-6 py-3 text-left text-xs font-medium text-brand-text uppercase tracking-wider">Flight</th>
              <th className="px-6 py-3 text-left text-xs font-medium text-brand-text uppercase tracking-wider">Confirmation</th>
              <th className="px-6 py-3 text-left text-xs font-medium text-brand-text uppercase tracking-wider">Document</th>
              <th className="px-6 py-3 text-left text-xs font-medium text-brand-text uppercase tracking-wider">Status</th>
              <th className="px-6 py-3 text-left text-xs font-medium text-brand-text uppercase tracking-wider">Actions</th>
            </tr>
          </thead>
            ...
            {checkIns.map((ci) => (
              <tr key={ci.id}>
                ...
                <td className="px-6 py-4 whitespace-nowrap">
                  {ci.documentUrl ? (
                    <a href={ci.documentUrl} target="_blank" rel="noopener noreferrer" className="text-brand-blue-600 hover:underline">
                      View Document
                    </a>
                  ) : (
                    'N/A'
                ...
                <td className="px-6 py-4 whitespace-nowrap">
                  <select
                    ...
                    className="block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-brand-blue-500 focus:border-brand-blue-500 sm:text-sm rounded-md"
                  >
